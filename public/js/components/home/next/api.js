...
var NextOsc = (function (_React$Component) {
    _inherits(NextOsc, _React$Component);

    function NextOsc(props) {
        _classCallCheck(this, NextOsc);

        _get(Object.getPrototypeOf(NextOsc.prototype), "constructor", this).call(this, props);
        this.state = {
            loadingAreas: false,
            loadingOscs: false,
            areaAtuacao: 0,
    ...
    _createClass(NextOsc, [{
        ...
        value: function callMenu() {
            ...
            var cd_municipio = this.state.cd_municipio;
            var geo = this.state.geo;
            var url = getBaseUrl2 + 'lista_por_area_atuacao/' + this.state.areaAtuacao;
            if (cd_municipio) {
        ...
        key: "setAreaAtuacao",
        value: function setAreaAtuacao(areaAtuacao, txt) {
        ...
        key: "setMunicipio",
        value: function setMunicipio(edmu_cd_municipio, edmu_nm_municipio, eduf_sg_uf) {
        ...
        value: function getLogos() {
            ...
            var _loop = function (i) {
                var id_osc = _this.state.nextsOsc[i].id_osc;
                $.ajax({
                    method: 'GET',
            ...
            for (var i in this.state.nextsOsc) {
                _loop(i);
            }
        }
    }, {
        ...
        value: function render() {
            ...
            var geoCity = localStorage.getItem('city') + ' - ' + localStorage.getItem('state');

            var municipios = this.state.municipios.map(function (item, index) {
                return React.createElement(
                    "li",
                    { style: { cursor: 'pointer' }, onClick: function () {
                            return _this2.setMunicipio(item.edmu_cd_municipio, item.edmu_nm_municipio, item.eduf_sg_uf);
                        }, key: 'mun' + index },
            ...
            if (this.state.data.length > 0) {
                owNextOsc = React.createElement(
                    ...
                    this.state.data.map((function (item, index) {
                        ...
                        var tx_nome_area_atuacao = item.tx_nome_area_atuacao == 'Associações patronais, profissionais e de produtores rurais' ? 'Associações' : item.tx_nome_area_atuacao;
                        ...
                        return React.createElement(
                            ...
                            React.createElement(
                                "a",
                                { onClick: function () {
                                        return _this3.setAreaAtuacao(item.cd_area_atuacao, item.tx_nome_area_atuacao);
                                    } },
            ...
            var nextOsc1 = null;
            var nextOsc2 = null;
            var nextOscTitulo = null;
            var totalnextsOsc = this.state.nextsOsc.length;

            var rotations = [[0, 0], [30, -30], [60, -60], [90, -90], [120, -120], [150, -150], [180, -180], [210, -210], [240, -240], [270, -270], [300, -300], [330, -330]];

            if (this.state.nextsOsc) {
                nextOsc1 = this.state.nextsOsc.map((function (item, index) {
                    var _this4 = this;

                    var logo = objTest(this.state.logos[item.id_osc]) ? this.state.logos[item.id_osc] : 'img/sem-imagem.png';
                    if (index <= 2) {
                        var random = Math.floor(Math.random() * rotations.length);
                        var rotation = rotations[random];
                        rotations.splice(random, 1); //remove do array o ratation utilizado.
                        return React.createElement(
                            "a",
                            { href: "detalhar/" + item.id_osc + "/" + clean(item.tx_nome_osc), id: 'icon' + index, className: "rotate", onClick: function () {
                                    return _this4.callMenu2(item.id_osc);
                                }, style: { transform: "rotate(" + rotation[0] + "deg)" }, key: 'listnext' + index },
                            React.createElement(
                                "div",
                ...
                nextOsc2 = this.state.nextsOsc.map((function (item, index) {
                    ...
                    if (index > 2) {
                        ...
                        return React.createElement(
                            "a",
                            { href: "detalhar/" + item.id_osc + "/" + clean(item.tx_nome_osc), id: 'icon' + index, className: "rotate", onClick: function () {
                                    return _this5.callMenu2(item.id_osc);
                                }, style: { transform: "rotate(" + rotation[0] + "deg)" }, key: 'listnext2' + index },
                            React.createElement(
                                "div",
                                { className: "circle-item", style: { transform: "rotate(" + rotation[1] + "deg)" } },
                                React.createElement("img", { src: logo, alt: "", className: "rounded-circle float-left", width: "60", height: "60" })
...
/*<img src="img/sem-imagem.png" alt={item.tx_nome_osc} width="65"/>*/ /*<img src="img/sem-imagem.png" alt={item.tx_nome_osc} width="65"/>*/ /*<h2>5 OSCs mais próximas de você, por área de atuação</h2>*/