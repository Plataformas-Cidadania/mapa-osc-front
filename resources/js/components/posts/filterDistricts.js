class DistrictsFilter extends React.Component{
    ...
    render(){
        ...
        let districts = this.state.districts.map(function (item){
            ...
            return (
                <div key={item.id} style={{cursor:'pointer', color: color}} onClick={() => this.addDistrict(item)}>
        ...
        let districtsSelected = this.state.districtsSelected.map(function (item){
            return (
                <button key={item.id} id={item.id} onClick={this.removeDistrict} type="button" className="btn btn-success btn-xs btn-remove" style={{margin: "0 5px 5px 0"}}>
        ...
        return(
            <div>
                {districtsSelected}
                <input type="text" name="titleDistrict" className="form-control input-sm" onClick={this.clickSearch} onChange={this.handleSearch}/>
                <div className="div-info" style={{border: "solid 1px #CCC", display: this.state.showDistricts ? 'block' : 'none'}}>
                    {districts}
                </div>
            </div>
        );
