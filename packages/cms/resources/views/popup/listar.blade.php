...
@section('content')
    {!! Html::script(config('app.url').'assets-cms/js/controllers/popupCtrl.js') !!}
<script>
    $(function () {
    ...
    <div ng-controller="popupCtrl">
        <div class="box-padrao">
            <h1><i class="fa fa-square" aria-hidden="true"></i>&nbsp;popups</h1>
            <button class="btn btn-primary" ng-click="mostrarForm=!mostrarForm" ng-show="!mostrarForm">Nova popup</button>
            ...
            <div class="col-md-12">
                <div class="box-padrao">
                    ...
                    <table ng-show="totalItens>0" class="table table-striped">
                        <thead>
                        <tr>
                            <th ng-click="ordernarPor('id')" style="popupr:pointer;">
                                Id
                            ...
                            <th>Imagem</th>
                            <th ng-click="ordernarPor('popup')" style="popupr:pointer;">
                                popup
                                <i ng-if="ordem=='popup' && sentidoOrdem=='asc'" class="fa fa-angle-double-down"></i>
                                <i ng-if="ordem=='popup' && sentidoOrdem=='desc'" class="fa fa-angle-double-up"></i>
                            </th>
                            <th ng-click="ordernarPor('posicao')" style="popupr:pointer;">
                                Posição
                        ...
                        <tr ng-repeat="popup in popups">
                            <td><% popup.id %></td>
                            <td><img ng-show="popup.imagem" ng-src="imagens/popups/xs-<% popup.imagem %>" width="60"></td>
                            <td><a href="cms/popup/<% popup.id %>"><% popup.titulo %></a></td>
                            <td><a href="cms/popup/<% popup.id %>"><% popup.posicao %></a></td>
                            <td class="text-right">
                                <div>
                                    <a><i class="fa fa-arrow-circle-up fa-2x" title="Posição" ng-click="positionUp(popup.id);" style="cursor: pointer;" ng-hide="<% $first %>"></i></a>
                                    <a><i class="fa fa-minus-circle fa-2x" title="Posição"   ng-show="<% $first %>" style="color: #CCCCCC; margin-right: 5px;"></i></a>&nbsp;&nbsp;

                                    <a><i class="fa fa-arrow-circle-down fa-2x" title="Posição" ng-click="positionDown(popup.id);"  style="cursor: pointer;" ng-hide="<% $last %>"></i></a>
                                    <a><i class="fa fa-minus-circle fa-2x" title="Posição"   ng-show="<% $last %>" style="color: #CCCCCC; margin-right: 5px;"></i></a>

                                    {{--<a href="cms/items/<% popup.id %>"><i class="fa fa-sitemap fa-2x" title="Itens"></i></a>&nbsp;&nbsp;--}}
                                    <a href="cms/popup/<% popup.id %>"><i class="fa fa-edit fa-2x" title="Editar"></i></a>&nbsp;&nbsp;
                                    <a  ng-class="<% popup.status %> == 1 ? 'color-success' : 'color-success-inactive'"  style="cursor: pointer;"><i class="fa fa-check-circle fa-2x" aria-hidden="true" ng-click="status(popup.id);"></i></a>
                                    <a><i data-toggle="modal" data-target="#modalExcluir" class="fa fa-remove fa-2x" ng-click="perguntaExcluir(popup.id, popup.titulo, popup.imagem)"></i></a>
                                </div>
                            </td>
                        </tr>
                        </tbody>
            ...
            <div class="col-md-12">
                ...
                <div ng-show="totalItens > 0" class="clan-paginacao">
                    <div class="item-paginacao">
                        <uib-pagination total-items="totalItens" ng-model="currentPage" max-size="maxSize" class="pagination-sm" boundary-popups="true" force-ellipses="true" items-per-page="itensPerPage" num-pages="numPages"></uib-pagination>
                    </div>
                    <div class="item-paginacao">
            ...
            <div class="modal-dialog">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-3">
                                <img  ng-src="imagens/popups/xs-<% imagemExcluir %>" width="100">
                            </div>
                            <div class="col-md-9">
                                <p><% tituloExcluir %></p>
                            </div>
