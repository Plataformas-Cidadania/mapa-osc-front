...
@section('content')
    {!! Html::script(config('app.url').'assets-cms/js/controllers/alterarSubitemCtrl.js') !!}
    <div ng-controller="alterarSubitemCtrl">
        <div class="box-padrao">
            <h1><a href="javascript:history.back();"><i class="fa fa-arrow-circle-left"></i></a>&nbsp;&nbsp;Subitems</h1>
            <?php //print_r($subitem);?>
            <div ng-init="carregaImagem('{{$subitem->imagem}}', '{{$subitem->arquivo}}')">
                <span class="texto-obrigatorio">* campos obrigatórios</span><br><br>
                {!! Form::model($subitem, ['name' =>'form']) !!}
                ...
                <a href="arquivos/subitems/<% arquivoBD %>" target="_blank" ng-show="arquivoBD"><% arquivoBD %></a>
                <a ng-show="fileArquivo"><% fileArquivo.name %></a>
                <br><br>

                <br><br>
                @include('cms::subitems._form')
                <input type="hidden" name="id" ng-model="id" ng-init="id='{{$subitem->id}}'"/>
                <div class="row">
                    <div class="col-md-1 col-lg-1 col-xs-3">
                        <button class="btn btn-info" type="button" ng-click="alterar(picFile, fileArquivo)" ng-disabled="form.$invalid && form.subitem.$dirty">Salvar</button>
                    </div>
