...
@section('content')
    {!! Html::script(config('app.url').'assets-cms/js/controllers/alterarPublicationCtrl.js') !!}
    <div ng-controller="alterarPublicationCtrl">
        <div class="box-padrao">
            <h1><a href="javascript:history.back();"><i class="fa fa-arrow-circle-left"></i></a>&nbsp;&nbsp;Publications</h1>
            <?php //print_r($publication);?>
            <div ng-init="carregaImagem('{{$publication->imagem}}', '{{$publication->arquivo}}')">
                <span class="texto-obrigatorio">* campos obrigatórios</span><br><br>
                {!! Form::model($publication, ['name' =>'form']) !!}
                ...
                <a href="arquivos/publications/<% arquivoBD %>" target="_blank" ng-show="arquivoBD"><% arquivoBD %></a>
                <a ng-show="fileArquivo"><% fileArquivo.name %></a>
                <br><br>

                <br><br>
                @include('cms::publication._form')
                <input type="hidden" name="id" ng-model="id" ng-init="id='{{$publication->id}}'"/>
                <div class="row">
